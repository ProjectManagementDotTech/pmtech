<template>
    <div class="relative">
        <button class="relative hover:bg-gray-800 block mt-1 rounded px-2 py-1 text-white font-semibold focus:outline-none focus:bg-gray-800 sm:ml-2 sm:mt-0"
            ...
            Clients<i class="ml-2 fas" :class="chevronClass"></i>
        </button>
        ...
        <div v-if="isOpen" class="absolute right-0 bg-white rounded-lg py-2 w-48 shadow-xl z-40">
            <a class="block px-4 py-2 hover:bg-gold-100 focus:bg-gold-100 focus:outline-none cursor-pointer" @click="onClickAddClient">Add</a>
            <router-link class="block px-4 py-2 hover:bg-gold-100 focus:bg-gold-100 focus:outline-none" :to="'/workspaces/' + $route.params.workspaceId + '/clients'" @click.native="isOpen = false">Overview</router-link>
        </div>
    </div>
</template>
